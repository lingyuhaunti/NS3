...
//                   MakeEnumChecker (CLOSED, "Closed"))
    .AddAttribute ("MaxSegLifetime",
                   ...
                   DoubleValue (0.1), /* RFC793 says MSL=2 minutes*/
    ...
    .AddAttribute ("MinRto",
                   ...
                   TimeValue (Seconds (0.2)), // RFC 6298 says min RTO=1 sec, but Linux uses 200ms.
  ...
  if (InetSocketAddress::IsMatchingType (address) && m_endPoint6 == 0)
    {
      ...
      SetIpTos (transport.GetTos ());
      // std::cout<<"Connect  "<<(transport.GetTos ()>>5)<<std::endl;
      m_endPoint6 = 0;
...
TcpSocketBase::Close (void)
{
  ...
  // std::cout<<"Close:"<<Simulator::Now ().GetMicroSeconds()<<"    peer:  "<<m_endPoint->GetPeerAddress ()<<":"<<m_endPoint->GetPeerPort ()
  // <<"    local:  "<<m_endPoint->GetLocalAddress ()<<":"<<m_endPoint->GetLocalPort ()<<std::endl;
                          ...
                          Ptr<Ipv4Interface> incomingInterface)
{
  ...
  // std::cout<<"  TcpSocketBase::ForwardUp  TOS=     "<< (header.GetTos()>>5)<<std::endl;
  // SocketPriorityTag priorityTag;
  // packet->PeekPacketTag (priorityTag);
  
  //std::cout<<priorityTag.GetPriority()<<std::endl;
                            ...
                            const Address &toAddress)
{
  ...
  // SocketPriorityTag priorityTag;
  // packet->PeekPacketTag (priorityTag);
  // // uint8_t pro=priorityTag.GetPriority();
  // std::cout<<priorityTag.GetPriority()<<std::endl;
...
void FlowComplete(uint32_t flow_id, const TcpHeader& tcpHeader){
  std::cout << "FlowComplete   "<< flow_id <<"   "<<++flowc_count<<endl;  
  m_static[flow_id].SetflowRsize(tcpHeader.GetAckNumber().GetValue());
  m_static[flow_id].SetflowEnd(Simulator::Now().GetMicroSeconds());
  
  output_file << m_static[flow_id].GetflowId() << "  " << m_static[flow_id].GetflowSsize() << "  " << m_static[flow_id].GetflowRsize() << "  " << m_static[flow_id].GetflowEnd() - m_static[flow_id].GetflowStart() << std::endl;
  
  if((m_static[flow_id].GetRecNode() % 4) == 3)
  {
    // next step P2W
    Array_W2P[((m_static[flow_id].GetRecNode() + 1)/4 - 1) * 16 + m_static[flow_id].GetSendNode()] = true;
    for(int i = 63; i >= 0 ; i--)
    {
      std::cout<<Array_W2P[i];
    }
    std::cout<<endl;
    uint16_t m_sum = 0 ;
    for(int i = 0; i < 16; i++)
    {
      m_sum += Array_W2P[((m_static[flow_id].GetRecNode() + 1)/4 - 1) * 16 + i] * pow(2,i);
    }
    std::cout<<Simulator::Now().GetSeconds()<<"  Send   "<<m_static[flow_id].GetSendNode()<<"  REC  "<<m_static[flow_id].GetRecNode()<<"   "<<m_sum<<endl;
    if(m_sum == 30583)
    {
      Array_PSend[((m_static[flow_id].GetRecNode() + 1)/4 - 1)] = true;
      for(int i = 0; i < 16; i++)
      {
        Array_W2P[((m_static[flow_id].GetRecNode() + 1)/4 - 1) * 16 + i] = false;
      }
    }
  }
  else
  {
    // next step W2P
    Array_P2W[m_static[flow_id].GetRecNode() * 4 + ((m_static[flow_id].GetSendNode() + 1 )/4 -1)] = true;
    uint8_t m_sum = 0;
    for(int i = 0; i < 4; i++)
    {
      m_sum += Array_P2W[m_static[flow_id].GetRecNode() * 4 + i] * pow(2,i);
    }
    if(m_sum == 15)
    {
      Array_WSend[m_static[flow_id].GetRecNode()] = true;
      for(int i = 0; i < 4; i++)
      {
        Array_P2W[m_static[flow_id].GetRecNode() * 4 + i] = false;
      }
    }
  }
}
    ...
    { // Process the ACK, and if in FIN_WAIT_1, conditionally move to FIN_WAIT_2
      ...
      if (m_state == FIN_WAIT_1 && m_txBuffer->Size () == 0
        ...
        { // This ACK corresponds to the FIN sent
          ...
          m_state = FIN_WAIT_2;
        // std::cout<<"ProcessWait :"<<Simulator::Now ().GetMicroSeconds()<<"    peer:  "<<m_endPoint->GetPeerAddress ()<<":"<<m_endPoint->GetPeerPort ()
        // <<"    local:  "<<m_endPoint->GetLocalAddress ()<<":"<<m_endPoint->GetLocalPort ()<<std::endl;
        
        m_start2end << "Simulator time:" <<Simulator::Now ().GetMicroSeconds() <<"   local:  "<<m_endPoint->GetLocalAddress ()<<":"<<m_endPoint->GetLocalPort ()
        <<"   peer:  "<<m_endPoint->GetPeerAddress ()<<":"<<m_endPoint->GetPeerPort () << "   ProcessWait" << endl;
        // m_start2end << '\r' << '\n'<< endl;

        // cout << Simulator::Now ().GetMicroSeconds() - m_GetStartTime() << endl; 

        m_fct << "fct:" << Simulator::Now ().GetMicroSeconds() - m_GetStartTime() <<"   local:  "<<m_endPoint->GetLocalAddress ()<<":"<<m_endPoint->GetLocalPort ()
        <<"   peer:  "<<m_endPoint->GetPeerAddress ()<<":"<<m_endPoint->GetPeerPort ()<<endl;
        // m_fct << '\r' << '\n'<< endl;

        FlowComplete(m_endPoint->GetPeerPort(), tcpHeader);
...
TcpSocketBase::DoPeerClose (void)
{
  ...
  if (m_shutdownSend)
    { // The application declares that it would not sent any more, close this socket
      Close ();
        // std::cout<<"Dopeerclose socket time:"<<Simulator::Now ().GetMicroSeconds()<<"    peer:  "<<m_endPoint->GetPeerAddress ()<<":"<<m_endPoint->GetPeerPort ()
        // <<"    local:  "<<m_endPoint->GetLocalAddress ()<<":"<<m_endPoint->GetLocalPort ()<<std::endl;
    }
  else
...
{
  // std::cout<<"SendEmptyPacket   "<< flags<<std::endl;
  NS_LOG_FUNCTION (this << (uint32_t)flags);
  // std::cout<<"SendEmptyPacket   "<< (uint32_t)flags <<std::endl;
  Ptr<Packet> p = Create<Packet> ();
  TcpHeader header;
  ...
  if (GetIpTos () || MarkEmptyPacket ())
    {
      ...
      // std::cout<<"SendEmptyPacket   "<< (uint32_t)tos <<std::endl;
      SocketIpTosTag ipTosTag;
      ipTosTag.SetTos (tos);
      p->AddPacketTag (ipTosTag);

        // uint8_t tmp;
        // tmp = tos|(flags & 0B01110000);
        // SetIpTos(tmp);
...
{ // Wrapper to protected function NotifyConnectionSucceeded() so that it can
  ...
  // std::cout<<Simulator::Now ().GetMicroSeconds() <<"   local:  "<<m_endPoint->GetLocalAddress ()<<":"<<m_endPoint->GetLocalPort ()
  // <<"   peer:  "<<m_endPoint->GetPeerAddress ()<<":"<<m_endPoint->GetPeerPort () <<"   ConnectionSucceeded" << std::endl;

  m_start2end << "Simulator time:" <<Simulator::Now ().GetMicroSeconds() <<"   local:  "<<m_endPoint->GetLocalAddress ()<<":"<<m_endPoint->GetLocalPort ()
  <<"   peer:  "<<m_endPoint->GetPeerAddress ()<<":"<<m_endPoint->GetPeerPort () <<"   ConnectionSucceeded" << endl;
  // m_start2end << '\r' << '\n'<< endl;

  // m_SetStartTime(Simulator::Now ().GetMicroSeconds());
  //cout << m_GetStartTime() << endl;
  
  /*std::m_file<<"ConnectionSucceeded:"<<Simulator::Now ().GetMicroSeconds()<<"    peer:  "<<m_endPoint->GetPeerAddress ()<<":"<<m_endPoint->GetPeerPort ()
  <<"    local:  "<<m_endPoint->GetLocalAddress ()<<":"<<m_endPoint->GetLocalPort ()<<std::endl;
  std::m_file << "\n";*/
...
TcpSocketBase::SendDataPacket (SequenceNumber32 seq, uint32_t maxSize, bool withAck)
{
  ...
  if (m_closeOnEmpty && (remainingData == 0))
    {
      flags |= TcpHeader::FIN;
        //       std::cout<<"SendPendingData :"<<Simulator::Now ().GetMicroSeconds()<<"    peer:  "<<m_endPoint->GetPeerAddress ()<<":"<<m_endPoint->GetPeerPort ()
        // <<"    local:  "<<m_endPoint->GetLocalAddress ()<<":"<<m_endPoint->GetLocalPort ()<<std::endl;
...
TcpSocketBase::EstimateRtt (const TcpHeader& tcpHeader)
{
          ...
          if (m_timestampEnabled && tcpHeader.HasOption (TcpOption::TS))
            {
              ...
              NS_LOG_INFO("output  rtt "<<m.GetMicroSeconds());
            }
          else
            {
  ...
  if (!m.IsZero ())     //m=1
    ...
    NS_LOG_INFO("output  rtt "<<m.GetMicroSeconds()<<"  rto  "<<m_rto<<"   last Rtt  "<<m_lastRtt);
}
  ...
  if (m_dataRetrCount == 0)
    {
      Address address;
      GetSockName(address);
      m_retry_fail<<Simulator::Now()<<"\t"<<InetSocketAddress::ConvertFrom(address).GetPort()<<std::endl;
...
// void
// TcpSocketBase::SendACK (uint8_t prio)
// {
//   NS_LOG_FUNCTION (this);
