...
BuildAppsTest (uint32_t k, std::string trace)
{
		...
		// while(!flowf.eof())
		{
				...
				if (true)
			{
				...
				m_static[port].SetflowId(port);
				m_static[port].SetflowStart(tflow*1000000);
				m_static[port].SetflowSsize(tempsize*packet_size);
				m_static[port].SetSendNode(nsender);
				m_static[port].SetRecNode(nreceiver); 
...
void BuildParaToWorkTraffic(uint64_t src)
{
		ApplicationContainer flows;
		ApplicationContainer sinkApps;
		Address sinkLocalAddress (InetSocketAddress (Ipv4Address::GetAny (), port_ai));
		PacketSinkHelper sinkHelper ("ns3::TcpSocketFactory", sinkLocalAddress);
		ApplicationContainer sinkApp = sinkHelper.Install (m_host);
		sinkApp.Start (Seconds (sink_start_time));
		sinkApp.Stop (Seconds (sink_stop_time));
		
		uint64_t nsender = src;
		uint64_t nreceiver = 1;
		uint32_t tempsize = 14783;
		...
		for (int i = 0; i < 16; i++)
		{
				...
				// Initialize On/Off Application with addresss of the receiver
				OnOffHelper source ("ns3::TcpSocketFactory", Address (InetSocketAddress(r_ipaddr, port_ai)));
				source.SetAttribute ("OnTime", StringValue ("ns3::ConstantRandomVariable[Constant=1]"));
				source.SetAttribute ("OffTime", StringValue ("ns3::ConstantRandomVariable[Constant=0]"));
				source.SetAttribute ("DataRate", StringValue ("10Gbps"));
				source.SetAttribute ("DataRate", DataRateValue (DataRate (link_data_rate)));
				source.SetAttribute ("PacketSize", UintegerValue (packet_size));
				InetSocketAddress tmp1 = InetSocketAddress (r_ipaddr, port_ai);
				...
				// Install sink 
				PacketSinkHelper sink("ns3::TcpSocketFactory", Address(InetSocketAddress(Ipv4Address::GetAny(), port_ai)));
				sinkApps.Add(sink.Install(m_host.Get(nreceiver)));
				...
				// Get the current flow
				Ptr<Application> the_flow = flows.Get(flows.GetN()-1);
				the_flow->SetStartTime(Time(Simulator::Now().GetSeconds()));
				flows.Stop(Seconds(global_stop_time));
				sinkApps.Start(Seconds(0));
				sinkApps.Stop(Seconds(global_stop_time));
				//m_start2end << nsender+20 << " " << nreceiver+20 << " " << 3 << " " << tempsize/1000 << " " << tflow << " " << global_stop_time+26 << endl;
				...
				m_static[port_ai].SetflowId(port_ai);
				m_static[port_ai].SetflowStart(Simulator::Now().GetSeconds()*1000000);
				m_static[port_ai].SetflowSsize(tempsize*packet_size);
				m_static[port_ai].SetSendNode(nsender);
				m_static[port_ai].SetRecNode(nreceiver);   
				port_ai++;
				//m_start2end << "First line: flow #src dst priority packet# start_time end_time" << endl;
			}
}

void BuildWorkToParaTraffic(uint64_t src)
{
		ApplicationContainer flows;
		ApplicationContainer sinkApps;
		Address sinkLocalAddress (InetSocketAddress (Ipv4Address::GetAny (), port_ai));
...
MonitorToTrace (void)
{
	...
	for(int i = 0; i < 4; i++)
	{
		if(Array_PSend[i] == 1)
		{
			Simulator::Schedule (MilliSeconds (32), &BuildParaToWorkTraffic, i*4+3);
			Array_PSend[i] = 0;
		}
	}
	for(int i = 0; i < 16; i++)
	{
		if(Array_WSend[i] == 1)
		{
			Simulator::Schedule (MilliSeconds (240), &BuildWorkToParaTraffic, i);
			Array_WSend[i] = 0;
		}
	}
	Simulator::Schedule (MicroSeconds (10), &MonitorToTrace);
}
...
main (int argc, char *argv[])
{
	...
	sink_stop_time = 3;//global_stop_time + 60.0;
